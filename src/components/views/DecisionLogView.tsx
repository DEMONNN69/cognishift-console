...
import { useDecisions, useLogInteraction } from "@/hooks/use-api";
import { StatusDot } from "@/components/StatusDot";
import { formatDateTime } from "@/lib/format";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import type { Decision, Action } from "@/types/api";

const ACTIONS: Action[] = ["seen", "ignored", "dismissed", "snoozed"];

function LogInteractionPanel({ userId, notificationId }: { userId: string; notificationId: string }) {
  const [result, setResult] = useState<string | null>(null);
  const logInteraction = useLogInteraction();

  function handle(action: Action) {
    logInteraction.mutate(
      { user_id: userId, notification_id: notificationId, action },
      { onSuccess: (data) => setResult(data.action) }
    );
  }

  return (
    <div className="border-t border-border pt-2 flex items-center gap-2 flex-wrap">
      <span className="text-muted-foreground">Log interaction:</span>
      {ACTIONS.map((a) => (
        <button
          key={a}
          className="h-5 px-2 border border-border rounded-sm text-xs hover:bg-secondary font-mono"
          onClick={() => handle(a)}
      ...
      {result && <span className="font-mono text-primary">✓ {result}</span>}
      {logInteraction.isError && (
        <span className="text-status-block text-xs">{(logInteraction.error as Error).message}</span>
      )}
    </div>
  );
}
                  ...
                  {expandedId === d.id && (
                    <TableRow key={`${d.id}-detail`} className="bg-secondary/30">
                      <TableCell colSpan={10} className="px-6 py-3">
                        ...
                        <LogInteractionPanel userId={d.user_id} notificationId={d.notification_id} />
                      </TableCell>
                    </TableRow>
                  )}
                </>
